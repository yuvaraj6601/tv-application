...
src/recognition/user_registry.py   — UserRegistry: identify_or_register(embedding, first_seen_at, distance_threshold, data_dir, age, gender) -> visitor_id; matches against an in-memory KnownFace cache (seeded from repository.get_known_faces at startup), mints a new UUID + calls daily_writer.write_new_visitor on no match (age/gender captured only at first registration, never updated on later re-identifications)
...
FACE_MATCH_DISTANCE_THRESHOLD      — cosine distance (1 - cosine similarity) for InsightFace embeddings; default 0.2 (2026-08-06: tuned 0.5→0.6→0.7 during real-webcam testing, each too strict/caused duplicate visitors, then set down to 0.4, then further to 0.2 per explicit request — the strictest value tried, confirmed knowingly against that tuning direction); lower = stricter/more duplicate visitors, higher = looser/more tolerant of angle-lighting variation
DATA_DIR
